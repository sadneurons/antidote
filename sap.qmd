...
#| layout-nrow: 1
# Decsriptive statistics of the Hip fracture population
library(tidyverse)
library(rms)
library(plotly)
library(fable)
...
ts_vol <- vol %>% mutate(my = tsibble::yearmonth(my)) %>% tsibble()

p <- vol |> ggplot() + geom_line(aes(x=my, y=ptnum)) +
  scale_x_date(date_labels = "%m-%Y") +
  labs(x="Month-Year of discharge", y="Number of patients",
       title="Patients presenting to Wythenshawe  with #NOF by month", 
       subtitle = "Data courtesy of the National Hip Fracture Database")
p

fable_1 <- ts_vol %>% model(  
    ets = ETS(ptnum),
    arima = ARIMA(ptnum),
    snaive = SNAIVE(ptnum)
  )
forecast_1 <- forecast(fable_1, h = "3 years")
autoplot(forecast_1, data=vol, level=NULL) +
  labs(x="Month-Year of discharge",
       title="ARIMA/ETS/SNAIVE models predicting patients presenting to  
              Wythenshawe with #NOF",
       subtitle = "Data courtesy of the National Hip Fracture Database",
       y="Number of patients")
forecast_1$.model

arima_means <- forecast_1 %>% filter(.model=='arima')
arima_mean <- mean(arima_means$.mean)
ets_means <- forecast_1 %>% filter(.model=='ets')
ets_mean <- mean(ets_means$.mean)
snaive_means <- forecast_1 %>% filter(.model=='snaive')
snaive_mean <- mean(snaive_means$.mean)
print_mean <- mean(c(snaive_mean, ets_mean, arima_mean)) |> round(2)
```
We conducted a time series forecast (ARIMA, ETS and Naive (SNAIVE)) based on discharges from Wythenshawe over the period 2011 - 2022. This suggests an expected mean number of presentations per month (simple average of 3 model estimates) of `r print_mean`. At this rate, we would expect ~40 x 24 = `r print_mean*24` patients to have surgical repair of fractured neck of femur over the 2-year recruitment course of the trial.
...
p <- acute_los |> ggplot() + geom_line(aes(x=my, y=acutelos)) +
  labs(x="Month-Year of discharge", 
       title="Length-of-stay (LOS) of patients presenting to Wythenshawe Hospital with #NOF by month", 
