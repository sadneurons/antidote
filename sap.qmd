...
fable_1 <- ts_vol %>% 
  model(  
    ...
    prophet = prophet(ptnum ~ growth("linear") + season("year", type = "multiplicative"))
  )
...
fable_2 <- ts_acute_los %>% model(  
  ...
  prophet = prophet(acutelos ~ growth("linear") + season("year", type = "multiplicative"))
)
...
fable_3 <- ts_ana %>% model(  
  ...
  prophet = prophet(ga ~ growth("linear") + season("year", type = "multiplicative"))
)
...
fable_4 <- ts_spinal%>% model(  
  ...
  prophet = prophet(spinal ~ growth("linear") + season("year", type = "multiplicative"))
)
